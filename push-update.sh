...
docker image build -f push-update-Dockerfile --no-cache -t dgdockerx-landing-update .
echo "...docker image build is complete."

# Tag the new image and push it to Docker Hub
docker login
docker tag dgdockerx-landing-update mcfatem/dgdockerx-landing:latest
docker push mcfatem/dgdockerx-landing:latest
